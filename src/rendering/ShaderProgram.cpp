...
bool parseIncludeLine(const std::string& line, std::string& includePath) {
    const std::size_t firstNonWhitespace = line.find_first_not_of(" \t");
    if (firstNonWhitespace == std::string::npos || line.compare(firstNonWhitespace, 8, "#include") != 0) {
        return false;
    }

    std::size_t quoteStart = line.find('"', firstNonWhitespace + 8);
    if (quoteStart == std::string::npos) {
        return false;
    }
    std::size_t quoteEnd = line.find('"', quoteStart + 1);
    if (quoteEnd == std::string::npos || quoteEnd <= quoteStart + 1) {
        return false;
    }

    includePath = line.substr(quoteStart + 1, quoteEnd - quoteStart - 1);
    return true;
}

std::string readShaderFileWithIncludes(const std::string& filePath, std::unordered_set<std::string>& includeStack) {
    if (includeStack.find(filePath) != includeStack.end()) {
        std::cerr << "Detected cyclic shader include: " << filePath << std::endl;
        return "";
    }
    ...
    includeStack.insert(filePath);

    std::ostringstream result;
    std::string line;
    const std::string currentDir = directoryOf(filePath);
    while (std::getline(file, line)) {
        std::string includePath;
        if (parseIncludeLine(line, includePath)) {
            const std::string resolvedPath = isAbsolutePath(includePath) ? includePath : joinPath(currentDir, includePath);
            result << readShaderFileWithIncludes(resolvedPath, includeStack);
        } else {
            result << line << '\n';
        }
    }

    includeStack.erase(filePath);
    return result.str();
}
...
std::string Shader::readFile(const char* filePath) {
    ...
    return readShaderFileWithIncludes(filePath, includeStack);
}
