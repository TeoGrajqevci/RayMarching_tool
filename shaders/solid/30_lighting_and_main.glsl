...
vec3 sampleSolidBackground(vec3 rd) {
...
vec3 color = vec3(0.0);
if(hitScene) {
vec3 geometricN = normalize(getNormal(hitPos, getAdaptiveNormalEpsilon(dist)));
vec3 V = normalize(ro - hitPos);
SDFResult hitRes = sceneSDFFull(hitPos, geometricN);
vec3 N = sampleNormalTexture(geometricN, hitRes.mat.normalTexIndex, hitPos);
if(dot(N, geometricN) < 0.0) {
N = geometricN;
}
...
float dispersion = clamp(hitRes.mat.dispersion, 0.0, 1.0);
...
vec3 directLighting = (diffuse + specular) * lightColor * (NdotL * safeDirectIntensity);

vec3 pointLighting = vec3(0.0);
int pointCount = clamp(uPointLightCount, 0, MAX_POINT_LIGHTS);
for(int pointLightIndex = 0;
pointLightIndex < pointCount;
++ pointLightIndex) {
...
vec3 pointDirect = (pointDiffuse + pointSpecular) *
    ...
    (pointNdotL * pointIntensity * pointScale);
...
vec3 refrR = refract(I, N, 1.0 / max(iorR, 1.0001));
vec3 refrG = refract(I, N, 1.0 / max(iorG, 1.0001));
vec3 refrB = refract(I, N, 1.0 / max(iorB, 1.0001));
if(dot(refrR, refrR) < 1e-6) {
refrR = reflectedFallback;
}
if(dot(refrG, refrG) < 1e-6) {
refrG = reflectedFallback;
}
if(dot(refrB, refrB) < 1e-6) {
refrB = reflectedFallback;
}

vec3 refrDirR = normalize(refrR);
vec3 refrDirG = normalize(refrG);
vec3 refrDirB = normalize(refrB);

vec3 transmittedR = sampleSolidBackground(refrDirR);
vec3 transmittedG = sampleSolidBackground(refrDirG);
vec3 transmittedB = sampleSolidBackground(refrDirB);
vec3 throughHitPos;
SDFResult throughHitRes;
if(marchSceneTransmission(hitPos + refrDirR * (hitEpsAtHit * 6.0), refrDirR, throughHitPos, throughHitRes, transmissionSamples)) {
vec3 throughBase = max(throughHitRes.mat.albedo, vec3(0.0));
vec3 throughEmission = max(throughHitRes.mat.emission, vec3(0.0)) * max(throughHitRes.mat.emissionStrength, 0.0);
transmittedR = ambientLight * throughBase + throughEmission;
}
if(marchSceneTransmission(hitPos + refrDirG * (hitEpsAtHit * 6.0), refrDirG, throughHitPos, throughHitRes, transmissionSamples)) {
vec3 throughBase = max(throughHitRes.mat.albedo, vec3(0.0));
vec3 throughEmission = max(throughHitRes.mat.emission, vec3(0.0)) * max(throughHitRes.mat.emissionStrength, 0.0);
transmittedG = ambientLight * throughBase + throughEmission;
}
if(marchSceneTransmission(hitPos + refrDirB * (hitEpsAtHit * 6.0), refrDirB, throughHitPos, throughHitRes, transmissionSamples)) {
vec3 throughBase = max(throughHitRes.mat.albedo, vec3(0.0));
vec3 throughEmission = max(throughHitRes.mat.emission, vec3(0.0)) * max(throughHitRes.mat.emissionStrength, 0.0);
transmittedB = ambientLight * throughBase + throughEmission;
}
transmitted = vec3(transmittedR.r, transmittedG.g, transmittedB.b);
}
...
color = sampleSolidBackground(rd);
}

if(uDebugOutputMode == DEBUG_OUTPUT_COUNTERS) {
int encodedTermination = clamp(terminationCode, 0, 255);
vec3 encodedCounters = vec3(float(clamp(mainMarchSteps, 0, 255)), 0.0, float(clamp(transmissionSamples, 0, 255))) / 255.0;
...
float gridDepth = tPlane;
if(uCameraProjectionMode == 1) {
gridDepth = abs(tPlane);
}
if(gridDepth > 0.0 && gridDepth < MAX_DIST) {
...
if(abs(pPlane.x) < axisThreshold) baseGridColor = mix(baseGridColor, vec3(0.0, 1.0, 0.0), 0.3);
if(abs(pPlane.z) < axisThreshold) baseGridColor = mix(baseGridColor, vec3(1.0, 0.0, 0.0), 0.3);
float fogDistance = (uCameraProjectionMode == 1) ? 45.0 : 25.0;
float fogFactor = clamp(1.0 - gridDepth / fogDistance, 0.0, 1.0);
if(gridDepth < dist) color = mix(color, baseGridColor, gridIntensity * fogFactor);
}
}
}
