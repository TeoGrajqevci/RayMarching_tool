...
void getArrayCandidateIndices(float pAxis, float spacing, float repeatCount, float smoothness, out ivec3 indices, out int count) {
...
float sdMandelbulbLocal(vec3 p, float power, float iterationsF, float bailout, float derivativeBias, float distanceScale) {
...
return vec3(catmullRomScalar(p0.x, p1.x, p2.x, p3.x, t), catmullRomScalar(p0.y, p1.y, p2.y, p3.y, t), catmullRomScalar(p0.z, p1.z, p2.z, p3.z, t));
}
...
if(shape.arrayAxis.x > 0.5) sampleP.x -= (float(xIndex) - centerX) * shape.arraySpacing.x;
if(shape.arrayAxis.y > 0.5) sampleP.y -= (float(yIndex) - centerY) * shape.arraySpacing.y;
if(shape.arrayAxis.z > 0.5) sampleP.z -= (float(zIndex) - centerZ) * shape.arraySpacing.z;
