...
const int DEBUG_OUTPUT_NONE = 0;
const int DEBUG_OUTPUT_COUNTERS = 1;
...
float getAdaptiveHitEpsilon(float t) {
    ...
    return clamp(max(SURFACE_DIST, pixelWorld * 0.65 * qualityScale), SURFACE_DIST * 0.75, SURFACE_DIST * 8.0);
}
...
float getSafeOverRelax(float dAbs, float hitEps) {
    ...
    return mix(1.0, 1.30, nearSurface);
}
